...
BEGIN
  ...
  --  END
    ...
    BEGIN
		...
		WHEN NOT MATCHED BY TARGET THEN
			INSERT (Email, UserName, NormalizedUserName, NormalizedEmail, IsEnabled, EmailConfirmed, PhoneNumberConfirmed, TwoFactorEnabled, LockoutEnabled, PasswordHash, AccessFailedCount, InstitutionId, SecurityStamp, ConcurrencyStamp, IsActive, CreatedBy, UpdatedBy, UpdatedDate, CreatedDate)
			...
			SUBSTRING(Source.AssociatedEmail, 1, 
				CASE 
					...
					ELSE LEN(Source.AssociatedEmail) -- Use the entire length if '@' is not found
				END
			)), 1, 1, 1, 0, 1, @PasswordHashed, 0, @InstitutionId, @SecurityStamp, @ConcurrencyStamp, 1, @CreatedBy, @CreatedBy, GETDATE(), GETDATE());
