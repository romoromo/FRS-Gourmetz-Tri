...
namespace BAL.Services
{
    public class ChannelInfoService : IChannelInfoService
    {
        ...
        public ChannelInfoService(IUnitOfWork uow, ISieveProcessor sieveProcessor)
        {
        ...
        {
            var result = Mapper.Map<PagedEntity<ChannelInfoDTO>>(await this._uow.ChannelInfos.GetChannelInfosAsync(filter));
            return result;
        }

        public async Task<ChannelInfoDTO> GetChannelInfoByIdAsync(int id)
        {
            return Mapper.Map<ChannelInfoDTO>(await this._uow.ChannelInfos.GetByIdAsync(id));
        }

        public async Task<BaseOperationResponse> CreateChannelInfoAsync(ChannelInfoDTO dto)
        {
            var result = new BaseOperationResponse();
            result = await this._uow.ChannelInfos.CreateAsync(Mapper.Map<ChannelInfo>(dto));
            return result;
        }
        ...
        {
            var result = new BaseOperationResponse();
            result = await this._uow.ChannelInfos.UpdateAsync(Mapper.Map<ChannelInfo>(dto));
            return result;
        }
