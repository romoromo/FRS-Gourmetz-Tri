...
namespace BAL.Services
{
    public class WalletService : IWalletService
    {
        ...
        public WalletService(IUnitOfWork uow, ISieveProcessor sieveProcessor)
        {
        ...
        {
            var result = Mapper.Map<PagedEntity<WalletDTO>>(await this._uow.Wallets.GetWalletsAsync(filter));
            return result;
        }

        public async Task<WalletDTO> GetByIdAsync(int id)
        {
            return Mapper.Map<WalletDTO>(await this._uow.Wallets.GetByIdAsync(id));
        }

        public async Task<List<WalletDTO>> GetByUserIdAsync(int userId)
        {
            return Mapper.Map<List<WalletDTO>>(await this._uow.Wallets.FindAsync(e => e.IsActive && e.UserId == userId));
        }

        public async Task<BaseOperationResponse> CreateAsync(WalletDTO dto)
        {
            var result = new BaseOperationResponse();
            result = await this._uow.Wallets.CreateAsync(Mapper.Map<Wallet>(dto));
            return result;
        }
        ...
        {
            var result = new BaseOperationResponse();
            result = await this._uow.Wallets.UpdateAsync(Mapper.Map<Wallet>(dto));
            return result;
        }
        ...
        {
            return Mapper.Map<List<WalletTransactionDTO>>((await this._uow.WalletTransactions.FindAsync(e => e.WalletId == id)).ToList());
        }
        ...
        public async Task<BaseOperationResponse> WalletOperationAsync(WalletOperationDTO dto)
        {
            ...
            else
            {
                ...
                var d = Mapper.Map<WalletDTO>(result.Data);
                result.Data = d;
            }
