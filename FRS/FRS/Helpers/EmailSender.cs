...
//            message.From = new MailAddress(config.EmailAddress, config.Name);
...
//            {
//                var emails = whitelist.Value.Split(';', StringSplitOptions.RemoveEmptyEntries).Select(e => e.Trim()).ToList();
...
//            catch (Exception ex)
//            {
//                Utilities.CreateLogger<EmailSender>().LogError(LoggingEvents.SEND_EMAIL, ex, "An error occurred while sending email");
