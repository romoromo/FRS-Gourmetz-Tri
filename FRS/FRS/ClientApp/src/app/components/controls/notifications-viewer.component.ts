...
export class NotificationsViewerComponent implements OnInit, OnDestroy {
  ...
  initDataLoading() {
    ...
    var filter = new Filter(1, 10);
      ...
      let dataLoadTask = this.notificationService.getNewNotifications(filter); //this.isViewOnly ? this.notificationService.getNewNotifications(filter) : this.notificationService.getNewNotificationsPeriodically(filter)
    ...
    this.notificationService.deleteNotification(row)
      .subscribe(results => {
        ...
        error => {
          ...
          this.alertService.showStickyMessage("Delete Error", `An error occured while deleting the notification.\r\nError: "${Utilities.getHttpResponseMessage(error)}"`,
    ...
    this.notificationService.pinUnpinNotification(row, pin)
      .subscribe(results => {
        ...
        error => {
          ...
          this.alertService.showStickyMessage(opText + " Error", `An error occured while ${opText} the notification.\r\nError: "${Utilities.getHttpResponseMessage(error)}"`,
            MessageSeverity.error);
        });
  }

  gotoNotificationsPage(ev) {
    ev.preventDefault();
