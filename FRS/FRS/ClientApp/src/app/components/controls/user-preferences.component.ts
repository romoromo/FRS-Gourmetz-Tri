  ...
  setAsDefaultHelper() {
    ...
    this.accountService.updateUserPreferences(this.configurations.export())
      .subscribe(response => {
        ...
        error => {
          this.alertService.stopLoadingMessage();
          this.alertService.showStickyMessage("Save Error", `An error occured while saving configuration defaults.\r\nErrors: "${Utilities.getHttpResponseMessage(error)}"`,
            MessageSeverity.error);
        });
  }
  ...
  resetDefaultHelper() {
    ...
    this.accountService.updateUserPreferences(null)
      .subscribe(response => {
        ...
        error => {
          this.alertService.stopLoadingMessage();
          this.alertService.showStickyMessage("Save Error", `An error occured while resetting configuration defaults.\r\nErrors: "${Utilities.getHttpResponseMessage(error)}"`,
            MessageSeverity.error);
        });
  }
