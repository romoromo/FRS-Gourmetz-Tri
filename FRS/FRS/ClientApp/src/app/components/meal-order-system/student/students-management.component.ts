...
export class StudentsManagementComponent implements OnInit, OnDestroy {
  ...
  getClassBatches() {
    ...
    this.subscription.add(
      this.classService.getClassBatchesByFilter(filter).subscribe(
        (results) => {
        ...
        (error) => {
          ...
          this.alertService.showStickyMessage(
            "Get Error",
            `An error occured while retrieving batches.\r\n"`,
  ...
  loadData(ev?: any) {
    ...
    this.filter.filters =
      f + "(IsActive)==true,(StudentPostSearch)@=" + this.keyword;

    this.subscription.add(
      this.studentService.getStudentsByFilter(this.filter).subscribe(
        (results) => {
  ...
  deleteStudent(row: Student) {
    this.alertService.showDialog(
      'Are you sure you want to delete the \"' + row.name + '\" student?',
      DialogType.confirm,
      () => this.deleteStudentHelper(row),
    );
  }
  ...
  addVoucher(row: Student) {
    this.alertService.showDialog(
      ...
      (val) => {
        if (!val) return;
        this.studentService.addStudentVoucher(row.id, val).subscribe(
          (response) => {
            this.loadingIndicator = false;
  ...
  public importedFile = (files) => {
    ...
    if (
      !confirm(
        `Are you sure to import file '${files[0].name}'? \nImport cannot be undone after the file uploaded.`,
      )
    )
      return;
    ...
    this.subscription.add(
      this.studentService.importFile<HttpEvent<Object>>(formData).subscribe(
        (event) => {
        ...
        (error) => {
          ...
          this.alertService.showStickyMessage(
            "Import Error",
            `Unable to import the file to the server.`,
            MessageSeverity.error,
          );
        },
      ),
  ...
  public importedStudentCardFile = (files) => {
    ...
    this.alertService.showDialog(
      ...
      (val) => {
        ...
        this.alertService.startLoadingMessage("Uploading...");
        this.studentService
          .importStudentCardFile<HttpEvent<Object>>(formData)
          .subscribe(
            (event) => {
              if (event.type === HttpEventType.UploadProgress) {
              }
            ...
            (error) => {
              ...
              if (
                this.fileStudentCardImport &&
                this.fileStudentCardImport.nativeElement
              ) {
  ...
  downloadTemplate() {
    ...
    window.open(
      getBaseUrl() +
        "/gateway/Download/FileByPath?filePath=/" +
  ...
  downloadTemplateWithStudent() {
    ...
    const fileName =
      ...
      "_Template with Student Information.xlsx";
  ...
  offBoarding(studentId: string) {
    this.alertService.showDialog(
      "Are you sure you want to off-board this student? Once confirmed, the student will be marked as inactive, and their wallet balance and points will be reset to zero.",
      DialogType.confirm,
      () => {
        this.alertService.startLoadingMessage("Processing top-up...");
        this.studentService
          .offBoardingStudent(studentId, this.accountService.currentUser.id)
          .subscribe({
            next: (response) => {
              this.alertService.stopLoadingMessage();
              this.loadingIndicator = false;
  ...
  downloadWalletTransaction(studentId: string) {
    this.alertService.startLoadingMessage("Downloading...");
    const fileName =
      moment().format("DDMMYYYY_hhmmss") + "_Student-Wallet-Transactions.xlsx";

    this.studentService.generateWalletTransaction(studentId).subscribe(
      (data) => {
        console.log(data);
        saveAs(data, fileName);
  ...
  get canImportStudents() {
    return this.accountService.userHasPermission(
      Permission.viewMOSOutletStudentMgtImportPermission,
    );
  }

  get canAddStudent() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionNew,
    );
  }

  get canEditStudent() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionEdit,
    );
  }

  get canDeleteStudent() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionDelete,
    );
  }

  get canCreateStudentAccount() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionCreateAccount,
    );
  }

  get canManageStudentOrderMgt() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionOrderMgt,
    );
  }

  get canManageStudentVoucher() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionVoucher,
    );
  }

  get canManageStudentNotification() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionNotification,
    );
  }

  get canManageStudentCalendar() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletStudentMgtPermissionCalendar,
    );
  }
}
...
export class CreateAccountMultiple {
    ...
    this.studentService.getStudentsByFilter(filter).subscribe(
      (results) => {
        ...
        this.page =
          this.page ||
          (this.pagedResult &&
            this.pagedResult.filter &&
            this.pagedResult.filter.page)
            ? this.pagedResult.filter.page
            : 1;
        this.prevPage = this.page;
  ...
  save() {
    ...
    this.studentService.createAccount(ids).subscribe(
  ...
  private saveSuccessHelper() {
    ...
    this.alertService.showMessage(
      "Success",
      `Accounts was created successfully`,
