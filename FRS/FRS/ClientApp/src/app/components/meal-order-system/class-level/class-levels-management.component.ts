...
export class ClassLevelsManagementComponent implements OnInit {
  ...
  initializeTableDefinition() {
    ...
    this.columns = [
      ...
      { prop: 'mealSessionName', name: 'Meal Session', width: 200 },
  ...
  loadData(ev?: any) {
    ...
    this.classService.getClassLevelsByFilter(this.filter)
      .subscribe(results => {
  ...
  deleteClassLevel(row: ClassLevel) {
    this.alertService.showDialog('Are you sure you want to delete the \"' + row.name + '\" class level?', DialogType.confirm, () => this.deleteClassLevelHelper(row));
  }
  ...
  deleteClassLevelHelper(row: ClassLevel) {
    ...
    this.classService.deleteClassLevel(row.id)
      .subscribe(results => {
        this.alertService.stopLoadingMessage();
        this.loadingIndicator = false;
