...
export class SortingAreaEditorComponent {
  ...
  constructor(private alertService: AlertService, private deliveryService: DeliveryService, private accountService: AccountService,
    public dialogRef: MatDialogRef<SortingAreaEditorComponent>, private mealService: MealService, private fileService: FileService,
    @Inject(MAT_DIALOG_DATA) public data: any) {
    if (typeof (data.SortingArea) != typeof (undefined)) {
      if(data.SortingArea.catererInfoId)
        this.catererInfoId = data.SortingArea.catererInfoId;
  ...
  private save() {
    ...
    if (this.isNewSortingArea) {
      this.deliveryService.newSortingArea(this.sortingAreaEdit).subscribe(sortingArea => this.saveSuccessHelper(sortingArea), error => this.saveFailedHelper(error));
    }
    else {
      this.deliveryService.updateSortingArea(this.sortingAreaEdit).subscribe(response => this.saveSuccessHelper(), error => this.saveFailedHelper(error));
    }
  }
  ...
  private saveSuccessHelper(sortingArea?: SortingArea) {
    ...
    if (this.isNewSortingArea)
      this.alertService.showMessage("Success", `\"${this.sortingAreaEdit.code}\" was created successfully`, MessageSeverity.success);
    else
      this.alertService.showMessage("Success", `Changes to \"${this.sortingAreaEdit.code}\" was saved successfully`, MessageSeverity.success);
