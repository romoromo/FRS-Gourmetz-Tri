...
@Component({
  selector: 'order-delivery-report',
  templateUrl: './order-delivery-report.component.html',
  styleUrls: ['./order-delivery-report.component.css']
})
export class OrderDeliveryReportComponent implements OnInit {
  stores: StoreInfo[];
  ...
  constructor(private http: HttpClient, private alertService: AlertService, private deliveryService: DeliveryService, public dialog: MatDialog,
    public dialogRef: MatDialogRef<OrderDeliveryReportComponent>,private menuService: MenuService,
  ...
  downloadReport() {
    ...
    this.menuService.downloadDeliveryOrderReport(insideFilter).subscribe(
      data => {
        console.log(data);
