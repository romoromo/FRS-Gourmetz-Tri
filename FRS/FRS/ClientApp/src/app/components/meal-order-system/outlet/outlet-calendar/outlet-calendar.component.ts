...
import { BlockUnblockDateModel, BlockUnblockOutletDateModel, MenuCycle, MenuCycleBlockedDate, OutletBlockedDate } from 'src/app/models/meal-order/menu-cycle.model';
...
import { OutletViewMenuCycleMenuComponent } from '../view-menu/view-menu.component';
...
@Component({
  selector: 'outlet-calendar',
  templateUrl: './outlet-calendar.component.html',
  styleUrls: ['./outlet-calendar.component.css'],
  animations: [fadeInOut],
  encapsulation: ViewEncapsulation.None,
})
export class OutletMenuCycleCalendarComponent implements OnInit, AfterViewInit {
  schedules: MenuCycle[] = [];
    ...
    this.schedules.forEach((event) => {
      ...
      rule.all().forEach((date) => {
        ...
        if (date >= new Date(new Date(event.startDate).setHours(0)) && date <= new Date(new Date(event.endDate).setHours(0))) {
          ...
          let outletBlockedDates = event.outletBlockedDates.filter(e => date.getTime() == new Date(e.effectiveDate).getTime());
          events.push({
            title: label,
            color,
            start: start,
  ...
  loadData(id?: string) {
    this.deliveryService.getOutletById(id)
      .subscribe(outlet => {
        ...
        //if (approvedOutlet) {
          this.menuService.getOutletMenuCyclesByFilter(id, this.catererId)
            .subscribe(results => {
              let menuCycles = results;
  ...
  blockDate(ev, day) {
    ...
    if (day && day.events && day.events.length > 0 && !day.events[0].isBlocked) {
      ...
      this.alertService.showDialog(`Are you sure you want to "${(isBlocked.length > 0 ? "unblock": "block")}" this date?`, DialogType.confirm, () => this.blockMenuCycleHelper(day, isBlocked.length > 0));
    }
  }
  ...
  blockMenuCycleHelper(day: any, isUnblock) {
    ...
    let model = new BlockUnblockOutletDateModel();
    model.isUnblock = isUnblock;
    model.blockedDates = blockedDates;
    this.menuService.blockunblockOutletDate(model)
      .subscribe(results => {
  ...
  openViewMenuDialog(menuCycle: MenuCycle, day): void {
    const dialogRef = this.dialog.open(OutletViewMenuCycleMenuComponent, {
      data: { header: 'Menu List', menuCycle: menuCycle, day, outletId: this.outletId, catererId: this.catererId },
      width: '600px'
    });
  }
