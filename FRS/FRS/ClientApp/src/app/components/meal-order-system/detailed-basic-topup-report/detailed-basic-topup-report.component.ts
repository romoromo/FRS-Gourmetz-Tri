...
import {
    DetailedBasicWalletTopUpFilter,
  Filter,
  PagedResult,
...
import { DetailedBasicWalletTopUpModel } from "src/app/models/detailed-basic-wallet-topup.model";
...
@Component({
  selector: "detailed-basic-wallet-topup-report",
  templateUrl: "./detailed-basic-topup-report.component.html",
  styleUrls: ["./detailed-basic-topup-report.component.css"],
  providers: [DecimalPipe, DatePipe],
})
export class DetailedBasicWalletTopUpReport implements OnInit {
  private subscription: Subscription = new Subscription();
  filter = new DetailedBasicWalletTopUpFilter();
  start = new Date();
  end = new Date();
  tstart = new Date();
  tend = new Date();
  ...
  columns: any[] = [];
  rows: DetailedBasicWalletTopUpModel[] = [];
  rowsCache: DetailedBasicWalletTopUpModel[] = [];
  ...
  initializeTableDefinition() {
    ...
    this.columns = [
      { prop: "transactionDate", name: "Transaction Date", sortable: false, pipe: {
      ...
      { prop: "outlet", name: "Outlet", sortable: false },
  ...
  loadData(ev?: any) {
    ...
    this.auditService.getDetailedBasicWalletTopUpReport(this.filter).subscribe(
     (results) => {
  ...
  downloadWalletTransaction() {
    ...
    const fileName =
     moment().format("DDMMYYYY_hhmmss") + "_Detailed-Basic-Wallet-Top-Up-Report.xlsx";

    this.auditService.generateDetailedBasicWalletTopUpReport(this.filter).subscribe(
     (data) => {
       console.log(data);
