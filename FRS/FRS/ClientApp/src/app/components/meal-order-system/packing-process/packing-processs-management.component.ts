...
@Component({
  selector: 'packing-processs-management',
...
export class PackingProcesssManagementComponent implements OnInit {
  ...
  editedPackingProcess: BentoAsset;
  sourcePackingProcess: BentoAsset;
  ...
  get canManagePackingProcesss() {
    return true; //this.accountService.userHasPermission(Permission.managePackingProcesssPermission)
  }

  cartonChanged() {
    clearTimeout(this.cartonTimeout)
    this.cartonTimeout = setTimeout(() => {
      this.cartonSearchs = this.cartonAssets.filter(x => x.code && this.cartonAssetCode && x.code.toLowerCase().includes(this.cartonAssetCode.toLowerCase()));

      if (this.autoAdd && this.cartonSearchs.length == 1) {
        this.addBento();
  ...
  addBento() {
    if (!this.cartonAssetCode || !this.bentoAssetCode) return; 
