...
import {
  OrderPortalBanner,
  OrderPortalContent,
} from "src/app/models/meal-order/order-portal-content.model";
...
export class OrderPortalContentEditorComponent {
  ...
  constructor(
    ...
    private portalService: OrderPortalService,
    private accountService: AccountService,
    public dialogRef: MatDialogRef<OrderPortalContentEditorComponent>,
  ...
  private save() {
    ...
    if (this.isNewOrderPortalContent) {
      this.portalService
        .newOrderPortalContent(this.portalContentEdit)
        .subscribe(
          (portalContent) => this.saveSuccessHelper(portalContent),
          (error) => this.saveFailedHelper(error)
        );
    } else {
      this.portalService
        .updateOrderPortalContent(this.portalContentEdit)
        .subscribe(
  ...
  private saveSuccessHelper(portalContent?: OrderPortalContent) {
    if (portalContent) Object.assign(this.portalContentEdit, portalContent);
    ...
    if (this.isNewOrderPortalContent)
      ...
      this.alertService.showMessage(
        "Success",
        `Changes to the content was saved successfully`,
        MessageSeverity.success
      );
  ...
  removeBanner(compo) {
    ...
    if (indx > -1) this.portalContentEdit.banners.splice(indx, 1);
  }

  public uploadBannerImageFinished = (event, banner) => {
    this.fileUploadResponse = event;
    banner.imageFilePath = this.fileUploadResponse
  ...
  public uploadBannerFinished = (event, banner) => {
    ...
    banner.filePath = this.fileUploadResponse
      ? this.fileUploadResponse.dbPath
      : null;
  };
  ...
  downloadFile(path) {
    ...
    const fullPath  = (getBaseUrl() + encodeURIComponent(normalizedPath).replace(/%2F/g, "/")).replace("webadmin/", "");
    window.open(fullPath, "_blank"); 
   }

  get canManageOrderPortalContents() {
    return this.accountService.userHasPermission(
      Permission.manageMOSOutletMgtPortalContentsPermission
    );
  }
}
