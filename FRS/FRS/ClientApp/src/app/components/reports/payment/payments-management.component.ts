...
@Component({
  selector: 'payments-management',
  templateUrl: './payments-management.component.html',
  styleUrls: ['./payments-management.component.css']
})
export class PaymentsReportManagementComponent implements OnInit, OnDestroy {
  ...
  initializeTableDefinition() {
    ...
    this.columns = [
      ...
      { prop: 'paymentGst', name: 'Total Gst' },
      { prop: 'subtotal', name: 'Subtotal' },
      { prop: 'paymentTransactionFee', name: 'Transaction Fee' },
  ...
  downloadFlattentResults() {
    const fileName = moment().format('DDMMYYYY_hhmmss') + '_PaymentReport.xlsx';
  ...
  get canManageAuthLogs() {
    return this.accountService.userHasPermission(Permission.viewMOSOutletMgtReportsOrderPaymentPermission)
  }
