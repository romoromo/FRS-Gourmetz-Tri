...
@Component({
  selector: 'auth-logs-management',
  templateUrl: './auth-logs-management.component.html',
  styleUrls: ['./auth-logs-management.component.css']
})
export class AuthLogsManagementComponent implements OnInit, OnDestroy {
  ...
  initializeTableDefinition() {
    ...
    this.columns = [
      ...
      { prop: 'createdDate', name: 'Date', pipe: new DateTimeOnlyPipe('en-SG'), canAutoResize: false, width: 200 },
    ];
  }
  ...
  loadData(ev?: any) {
    ...
    this.filter.filters = '(UserName|Message)@=' + this.keyword + ',(AuditAuthDateRange)==' + this.start.toDateString() + '|' + this.end.toDateString();
    
    this.authLogService.getAuthLogsByFilter(this.filter)
      .subscribe(results => {
        ...
        error => {
          ...
          this.alertService.showStickyMessage("Load Error", `Unable to retrieve authentication logs from the server.\r\nErrors: "${Utilities.getHttpResponseMessage(error)}"`,
  ...
  downloadResults() {
    const fileName = moment().format('DDMMYYYY_hhmmss') + '_AuthLogs.xlsx';

    this.authLogService.downloadAuthLogReport(this.filter).subscribe(
      data => {
        console.log(data);
  ...
  get canManageAuthLogs() {
    return this.accountService.userHasPermission(Permission.viewAuthLogsPermission)
  }
