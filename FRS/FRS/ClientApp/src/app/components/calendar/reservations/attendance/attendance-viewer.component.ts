...
export class AttendanceViewerComponent implements OnInit {
  ...
  constructor(private cdRef: ChangeDetectorRef, private alertService: AlertService, private accountService: AccountService, private reservationService: ReservationService, private locationService: LocationService,
    public dialogRef: MatDialogRef<AttendanceViewerComponent>,
  ...
  private save() {
    ...
    if (this.reservationEdit.isAttendee && isAttendeeExist) {
      var resInvitee = new ReservationInvitee();
    ...
    this.accountService.refreshLoggedInUser()
      .subscribe(user => { },
        error => {
          ...
          this.alertService.showStickyMessage("Refresh failed", "An error occured while refreshing logged in user information from the server", MessageSeverity.error, error);
        });
  }
  ...
  loadLocations() {
    ...
    //    error => {
    //      this.alertService.resetStickyMessage();
    //      this.alertService.showStickyMessage("getLocationsByLocationId failed", "An error occured while trying to get locations from the server", MessageSeverity.error, error);
    //    });
  }
