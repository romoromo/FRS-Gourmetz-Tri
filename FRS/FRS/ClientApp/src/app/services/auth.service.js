...
var AuthService = /** @class */ (function () {
    ...
    Object.defineProperty(AuthService.prototype, "isEnableMFAValidation", {
        get: function () { return this.configurations.enableMFAValidation; },
        enumerable: false,
        configurable: true
    });
    ...
    AuthService.prototype.login = function (userName, password, institutionCode, rememberMe, isAD, isValidateMfa) {
        var _this = this;
        if (this.isLoggedIn)
            this.logout();
        return this.endpointFactory.getLoginEndpoint(userName, password, institutionCode, 'false', isAD).pipe(operators_1.map(function (response) { return _this.processLoginResponse(response, rememberMe, isValidateMfa); }));
    };
    ...
    AuthService.prototype.getResendCode2FA = function (userId) {
        var _this = this;
        return this.endpointFactory.getResendCode2FAEndpoint(userId).pipe(operators_1.map(function (response) { return _this.processResend2FAResponse(response); }));
    };
    ...
    AuthService.prototype.getUserPermissions = function (roleNames) {
        var _this = this;
        return this.endpointFactory.getUserPermissions(roleNames).pipe(operators_1.map(function (permissions) {
            console.log(permissions);
            _this.saveUserPermissionsDetails(permissions);
    ...
    AuthService.prototype.processResend2FAResponse = function (response) {
        if (!response.sent)
            throw new Error("An error has occurred. Unable to resend code.");
        return true;
    };
    AuthService.prototype.processLoginResponse = function (response, rememberMe, isValidateMfa) {
    ...
    AuthService.prototype.saveUserPermissionsDetails = function (permissions) {
        this.localStorage.savePermanentData(permissions, db_Keys_1.DBkeys.USER_PERMISSIONS);
    };
    ...
    AuthService.prototype.reevaluateLoginStatus = function (currentUser, isValidateMfa) {
        var _this = this;
        var d = new Date();
