...
export class MenuService {
  ...
  // individual order
  bulkStudentGroupOrder(studentGroupId: string, outletId: string, storeId: string, deliveryDate: string, deliveryDateTo: string, dishTypeId: string, mealSessionId: string, createdBy: string, isClearOrder: boolean, type: string, mealSessionDetailId: string) {
    return this.commonEndpoint.get<any>(`${this.tokenOrderUrl}/studentgroup/order?studentGroupId=${studentGroupId}&outletId=${outletId}&storeId=${storeId}&deliveryDate=${deliveryDate}&deliveryDateTo=${deliveryDateTo}&dishTypeId=${dishTypeId}&mealSessionId=${mealSessionId}&createdBy=${createdBy}&clear=${isClearOrder}&type=${type}&mealSessionDetailId=${mealSessionDetailId}`);
  }
