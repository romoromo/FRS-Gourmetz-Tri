...
namespace DAL.Core.Audit.Configuration
{
    public class OverrideTrackingResponse<T>
    {
        ...
        internal static void SkipProperty(Expression<Func<T, object>> property)
        {
            ...
            //if(property.GetType().Namespace == @"System.Linq.Expressions" && property.Parameters != null && property.Parameters.Count > 0)
            //{
            //    TrackingDataStore.PropertyConfigStore.AddOrUpdate(
            //    new PropertyConfigurationKey(info.Name, property.Parameters[0].Type.FullName),
            //    newValue,
            //    (existingKey, existingvalue) => newValue);
            //}
